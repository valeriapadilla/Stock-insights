...
func (jm *JobManager) CreateJob() (*model.Job, error) {
	job := &model.Job{
		ID:        uuid.New().String(),
...
func (jm *JobManager) GetJob(jobID string) (*model.Job, bool) {
	jm.mutex.RLock()
...
func (jm *JobManager) UpdateJob(jobID string, status model.JobStatus, progress int, message string) {
	jm.mutex.Lock()
	defer jm.mutex.Unlock()
	...
	if job, exists := jm.jobs[jobID]; exists {
		...
		if status == model.JobStatusRunning && job.StartedAt == nil {
			now := time.Now()
			job.StartedAt = &now
		} else if (status == model.JobStatusCompleted || status == model.JobStatusFailed) && job.EndedAt == nil {
			now := time.Now()
			job.EndedAt = &now
		}
...
func (jm *JobManager) RunJobAsync(jobID string, workFunc func(context.Context) error) error {
	...
	go func() {
		...
		jm.UpdateJob(jobID, model.JobStatusRunning, 0, "Starting job...")

		if err := workFunc(ctx); err != nil {
			jm.SetJobError(jobID, err)
		} else {
			jm.UpdateJob(jobID, model.JobStatusCompleted, 100, "Job completed successfully")
		}
	}()
